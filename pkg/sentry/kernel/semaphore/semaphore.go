...
// GetStatAny extracts semid_ds inforamtion from the set without requiring read access.
...
func (s *Set) semStat(creds *auth.Credentials, permMask fs.PermMask) (*linux.SemidDS, error) {
	// The calling process must have required permission on the semaphore set.
