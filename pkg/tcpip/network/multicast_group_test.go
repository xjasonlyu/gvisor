...
func createAndInjectIGMPPacket(e *channel.Endpoint, igmpType byte, maxRespTime byte, groupAddress tcpip.Address) {
	...
	optionsLength := int(options.Length())
	buf := buffer.NewView(header.IPv4MinimumSize + optionsLength + header.IGMPQueryMinimumSize)
	ip := header.IPv4(buf)
	...
	igmp := header.IGMP(buf[header.IPv4MinimumSize+optionsLength:])
