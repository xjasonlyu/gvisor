...
func TestExternalLoopbackTraffic(t *testing.T) {
	...
	loopbackSourcedICMPv4 := func(e *channel.Endpoint) {
		rxICMPv4(e, ipv4Loopback, ipv4Addr.Address)
	}

	loopbackSourcedICMPv6 := func(e *channel.Endpoint) {
		rxICMPv6(e, ipv6Loopback, ipv6Addr.Address)
	}

	loopbackDestinedICMPv4 := func(e *channel.Endpoint) {
		rxICMPv4(e, remoteIPv4Addr, ipv4Loopback)
	}

	loopbackDestinedICMPv6 := func(e *channel.Endpoint) {
		rxICMPv6(e, remoteIPv6Addr, ipv6Loopback)
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if err := s.AddAddressWithPrefix(nicID1, ipv4.ProtocolNumber, ipv4Addr); err != nil {
				t.Fatalf("AddAddressWithPrefix(%d, %d, %s): %s", nicID1, ipv4.ProtocolNumber, ipv4Addr, err)
			}
			if err := s.AddAddressWithPrefix(nicID1, ipv6.ProtocolNumber, ipv6Addr); err != nil {
				t.Fatalf("AddAddressWithPrefix(%d, %d, %s): %s", nicID1, ipv6.ProtocolNumber, ipv6Addr, err)
			}
