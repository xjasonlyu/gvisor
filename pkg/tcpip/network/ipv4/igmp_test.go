...
func createAndInjectIGMPPacket(e *channel.Endpoint, igmpType header.IGMPType, maxRespTime byte, ttl uint8, srcAddr, dstAddr, groupAddress tcpip.Address, hasRouterAlertOption bool) {
	...
	ipHeaderLength := header.IPv4MinimumSize + int(options.Length())
	buf := buffer.NewView(ipHeaderLength + header.IGMPQueryMinimumSize)
	...
	igmp := header.IGMP(buf[ipHeaderLength:])
...
func TestIGMPPacketValidation(t *testing.T) {
	tests := []struct {
		...
		stackAddr                tcpip.AddressWithPrefix
	...
	}{
		{
			...
			stackAddr:                tcpip.AddressWithPrefix{Address: stackAddr, PrefixLen: 24},
			srcAddr:                  remoteAddr,
			ttl:                      1,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if err := s.AddAddressWithPrefix(nicID, ipv4.ProtocolNumber, test.stackAddr); err != nil {
				t.Fatalf("AddAddressWithPrefix(%d, %d, %s): %s", nicID, ipv4.ProtocolNumber, test.stackAddr, err)
			}
			createAndInjectIGMPPacket(e, test.messageType, 0, test.ttl, test.srcAddr, test.stackAddr.Address, header.IPv4AllSystems, test.includeRouterAlertOption)
			stats := s.Stats()
			...
			var expectedValidCount, expectedInvalidCount uint64
			if test.expectValidIGMP {
				expectedValidCount = 1
			} else {
				expectedInvalidCount = 1
			}
			if got := test.getMessageTypeStatValue(s.Stats()); got != expectedValidCount {
				t.Errorf("got test.getMessageTypeStatValue(s.Stats()) = %d, want = %d", got, expectedValidCount)
			}
