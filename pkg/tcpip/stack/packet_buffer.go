...
// ReadFromData moves at most first count bytes from srcData to the end of d and
// returns the number of bytes moved.
func (d PacketData) ReadFromData(srcData PacketData, count int) int {
	return srcData.pk.data.ReadToVV(&d.pk.data, count)
}

// ReadFromVV moves at most first count bytes from srcVV to the end of d and
// returns the number of bytes moved.
...
// Range represents a subportion of a PacketBuffer.
type Range struct {
	pk     *PacketBuffer
	offset int
	length int
...
func (r Range) ToOwnedView() buffer.View {
	...
	proc := offsetLengthProc{
		offset: r.offset,
		length: r.length,
	}
	// Header portion.
	for i := range r.pk.headers {
		all = append(all, proc.next(r.pk.headers[i].buf)...)
		if proc.isDone() {
			break
		}
	}
...
func (r Range) Checksum() uint16 {
	...
	// Header portion.
	for i := range r.pk.headers {
		c.Add(proc.next(r.pk.headers[i].buf))
		if proc.isDone() {
			break
		}
	}
...
type offsetLengthProc struct {
	offset int
	length int
}

func (p *offsetLengthProc) isDone() bool {
	return p.length == 0
}

func (p *offsetLengthProc) next(b []byte) (toProc []byte) {
	if p.offset >= len(b) {
		p.offset -= len(b)
		return nil
	}
