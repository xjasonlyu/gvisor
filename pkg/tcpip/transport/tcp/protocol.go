...
func (*protocol) Segment(pkt *stack.PacketBuffer, opts stack.SegmentOptions) (stack.PacketBufferList, tcpip.Error) {
	...
	for data.Size() > 0 {
		...
		case header.IPv4ProtocolNumber:
			h := header.IPv4(newNetHdr)
			h.SetTotalLength(uint16(pb.Size()))
			h.SetChecksum(0)
			h.SetChecksum(^h.CalculateChecksum())
		case header.IPv6ProtocolNumber:
			h := header.IPv6(newNetHdr)
			h.SetPayloadLength(uint16(pb.Size() - int(header.IPv6FixedHeaderSize)))
		default:
			panic(fmt.Sprintf("unexpected network protocol number = %d", pb.NetworkProtocolNumber))
		}

		t := header.TCP(newTransportHdr)
		t.SetSequenceNumber(t.SequenceNumber() + uint32(done))
		t.SetChecksum(0)
		xsum := header.PseudoHeaderChecksum(ProtocolNumber, net.SourceAddress(), net.DestinationAddress(), uint16(segData.Size()+transportHdr.Size()))
